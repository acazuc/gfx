...
#define D3D9_DEVICE ((gfx_d3d9_device_t*)device)

typedef struct gfx_d3d9_device_s
{
	gfx_d3d_device_t device;
...
static const D3DPRIMITIVETYPE primitive_types[3] = {D3DPT_TRIANGLELIST, D3DPT_POINTLIST, D3DPT_POINTLIST};

static const D3DCMPFUNC compare_functions[8] = {D3DCMP_NEVER, D3DCMP_LESS, D3DCMP_LESSEQUAL, D3DCMP_EQUAL, D3DCMP_GREATEREQUAL, D3DCMP_GREATER, D3DCMP_NOTEQUAL, D3DCMP_ALWAYS};

static const D3DBLEND blend_functions[12] = {D3DBLEND_ZERO, D3DBLEND_ONE, D3DBLEND_SRCCOLOR, D3DBLEND_INVSRCCOLOR, D3DBLEND_DESTCOLOR, D3DBLEND_INVDESTCOLOR, D3DBLEND_SRCALPHA, D3DBLEND_INVSRCALPHA, D3DBLEND_DESTALPHA, D3DBLEND_INVDESTALPHA, D3DBLEND_BLENDFACTOR, D3DBLEND_INVBLENDFACTOR};

static const D3DBLENDOP blend_equations[5] = {D3DBLENDOP_ADD, D3DBLENDOP_SUBTRACT, D3DBLENDOP_REVSUBTRACT, D3DBLENDOP_MIN, D3DBLENDOP_MAX};

static const D3DSTENCILOP stencil_operations[8] = {D3DSTENCILOP_KEEP, D3DSTENCILOP_ZERO, D3DSTENCILOP_REPLACE, D3DSTENCILOP_INCRSAT, D3DSTENCILOP_INCR, D3DSTENCILOP_DECRSAT, D3DSTENCILOP_DECR, D3DSTENCILOP_INVERT};

static const D3DFILLMODE fill_modes[3] = {D3DFILL_POINT, D3DFILL_WIREFRAME, D3DFILL_SOLID};

static const D3DTEXTUREADDRESS texture_addressings[5] = {D3DTADDRESS_CLAMP, D3DTADDRESS_WRAP, D3DTADDRESS_MIRROR, D3DTADDRESS_BORDER, D3DTADDRESS_MIRRORONCE};

static const D3DTEXTUREFILTERTYPE filterings[3] = {D3DTEXF_NONE, D3DTEXF_POINT, D3DTEXF_LINEAR};
...
static void d3d9_dtr(gfx_device_t *device)
{
	(void)device;
}

static void d3d9_tick(gfx_device_t *device)
{
	(void)device;
}

static void d3d9_clear_color(gfx_device_t *device, const gfx_render_target_t *render_target, enum gfx_render_target_attachment attachment, vec4f_t color)
{
	uint32_t argb;
...
static void d3d9_clear_depth_stencil(gfx_device_t *device, const gfx_render_target_t *render_target, float depth, uint8_t stencil)
{
	if (render_target)
...
static void d3d9_draw_indexed_instanced(gfx_device_t *device, enum gfx_primitive_type primitive, uint32_t count, uint32_t offset, uint32_t prim_count)
{
	(void)prim_count;
	D3D9_DEVICE->d3ddev->DrawIndexedPrimitive(primitive_types[primitive], 0, 0, 0, offset, count);
}

static void d3d9_draw_instanced(gfx_device_t *device, enum gfx_primitive_type primitive, uint32_t count, uint32_t offset, uint32_t prim_count)
{
	(void)prim_count;
	D3D9_DEVICE->d3ddev->DrawPrimitive(primitive_types[primitive], offset, count);
}

static void d3d9_draw_indexed(gfx_device_t *device, enum gfx_primitive_type primitive, uint32_t count, uint32_t offset)
{
	D3D9_DEVICE->d3ddev->DrawIndexedPrimitive(primitive_types[primitive], 0, 0, 0, offset, count);
}

static void d3d9_draw(gfx_device_t *device, enum gfx_primitive_type primitive, uint32_t count, uint32_t offset)
{
	D3D9_DEVICE->d3ddev->DrawPrimitive(primitive_types[primitive], offset, count);
}

static void d3d9_create_blend_state(gfx_device_t *device, gfx_blend_state_t *state, bool enabled, enum gfx_blend_function src_c, enum gfx_blend_function dst_c, enum gfx_blend_function src_a, enum gfx_blend_function dst_a, enum gfx_blend_equation equation_c, enum gfx_blend_equation equation_a)
{
...
static void d3d9_bind_blend_state(gfx_device_t *device, const gfx_blend_state_t *state)
{
	assert(state->handle.u64);
...
static void d3d9_delete_blend_state(gfx_device_t *device, gfx_blend_state_t *state)
{
	(void)device;
	if (!state)
		return;
...
static void d3d9_bind_depth_stencil_state(gfx_device_t *device, const gfx_depth_stencil_state_t *state)
{
	assert(state->handle.u64);
...
static void d3d9_delete_depth_stencil_state(gfx_device_t *device, gfx_depth_stencil_state_t *state)
{
	(void)device;
	if (!state)
		return;
...
static void d3d9_create_rasterizer_state(gfx_device_t *device, gfx_rasterizer_state_t *state, enum gfx_fill_mode fill_mode, enum gfx_cull_mode cull_mode, enum gfx_front_face front_face, bool scissor)
{
	assert(!state->handle.u64);
	state->device = device;
	state->handle.u64 = ++D3D9_DEVICE->state_idx;
...
static void d3d9_bind_rasterizer_state(gfx_device_t *device, const gfx_rasterizer_state_t *state)
{
	assert(state->handle.u64);
...
static void d3d9_delete_rasterizer_state(gfx_device_t *device, gfx_rasterizer_state_t *state)
{
	(void)device;
	if (!state)
		return;
...
static void d3d9_create_buffer(gfx_device_t *device, gfx_buffer_t *buffer, enum gfx_buffer_type type, const void *data, uint32_t size, enum gfx_buffer_usage usage)
{
...
static void d3d9_set_buffer_data(gfx_device_t *device, gfx_buffer_t *buffer, const void *data, uint32_t size, uint32_t offset)
{
	assert(buffer->handle.ptr);
	void *ptr;
	switch (buffer->type)
...
static void d3d9_delete_buffer(gfx_device_t *device, gfx_buffer_t *buffer)
{
...
static void d3d9_create_attributes_state(gfx_device_t *device, gfx_attributes_state_t *state)
{
}

static void d3d9_bind_attributes_state(gfx_device_t *device, const gfx_attributes_state_t *state, const gfx_program_t *program)
{
}

static void d3d9_bind_attribute(gfx_device_t *device, gfx_attributes_state_t *state, uint32_t bind, const gfx_buffer_t *buffer, enum gfx_attribute_type type, uint32_t stride, uint32_t offset)
{
}

static void d3d9_bind_index(gfx_device_t *device, gfx_attributes_state_t *state, const gfx_buffer_t *buffer, enum gfx_index_type type)
{
}

static void d3d9_delete_attributes_state(gfx_device_t *device, gfx_attributes_state_t *state)
{
}

static void d3d9_create_texture(gfx_device_t *device, gfx_texture_t *texture, enum gfx_texture_type type, enum gfx_format format, uint8_t lod, uint32_t width, uint32_t height, uint32_t depth)
{
}

static void d3d9_set_texture_data(gfx_device_t *device, gfx_texture_t *texture, uint8_t lod, uint32_t offset, uint32_t width, uint32_t height, uint32_t depth, uint32_t size, const void *data)
{
}

static void d3d9_set_texture_addressing(gfx_device_t *device, gfx_texture_t *texture, enum gfx_texture_addressing addressing_s, enum gfx_texture_addressing addressing_t, enum gfx_texture_addressing addressing_r)
{
	(void)device;
...
static void d3d9_set_texture_filtering(gfx_device_t *device, gfx_texture_t *texture, enum gfx_filtering min_filtering, enum gfx_filtering mag_filtering, enum gfx_filtering mip_filtering)
{
	(void)device;
	assert(texture->handle.u64);
...
static void d3d9_set_texture_anisotropy(gfx_device_t *device, gfx_texture_t *texture, uint32_t anisotropy)
{
	(void)device;
	assert(texture->handle.u64);
...
static void d3d9_set_texture_levels(gfx_device_t *device, gfx_texture_t *texture, uint32_t min_level, uint32_t max_level)
{
	(void)device;
	assert(texture->handle.u64);
}

static void d3d9_delete_texture(gfx_device_t *device, gfx_texture_t *texture)
{
}

static void d3d9_create_render_buffer(gfx_device_t *device, gfx_render_buffer_t *render_buffer)
{
}

static void d3d9_set_render_buffer_storage(gfx_device_t *device, gfx_render_buffer_t *render_buffer, enum gfx_format format, uint32_t width, uint32_t height, uint32_t color_samples, uint32_t coverage_samples)
{
}

static bool d3d9_create_shader(gfx_device_t *device, gfx_shader_t *shader, enum gfx_shader_type type, const uint8_t *data, uint32_t len)
{
}

static void d3d9_delete_shader(gfx_device_t *device, gfx_shader_t *shader)
{
}

static bool d3d9_create_program(gfx_device_t *device, gfx_program_t *program, const gfx_shader_t *vertex_shader, const gfx_shader_t *fragment_shader, const gfx_shader_t *geometry_shader, const gfx_program_attribute_t *attributes, const gfx_program_constant_t *constants, const gfx_program_sampler_t *samplers)
{
}

static void d3d9_bind_program(gfx_device_t *device, const gfx_program_t *program)
{
}

static void d3d9_delete_program(gfx_device_t *device, gfx_program_t *program)
{
}

static void d3d9_bind_constant(gfx_device_t *device, uint32_t bind, const gfx_buffer_t *buffer, uint32_t size, uint32_t offset)
{
}

static void d3d9_bind_samplers(gfx_device_t *device, uint32_t start, uint32_t count, const gfx_texture_t **textures)
{
...
static void d3d9_create_render_target(gfx_device_t *device, gfx_render_target_t *render_target)
{
}

static void d3d9_delete_render_target(gfx_device_t *device, gfx_render_target_t *render_target)
{
}

static void d3d9_bind_render_target(gfx_device_t *device, const gfx_render_target_t *render_target)
{
}

static void d3d9_blit_render_target(gfx_device_t *device, const gfx_render_target_t *src, vec2f_t src_pos, vec2f_t src_size, const gfx_render_target_t *dst, vec2f_t dst_pos, vec2f_t dst_size, uint32_t buffers, uint32_t color_src, uint32_t color_dst, bool linear)
{
}

static void d3d9_set_render_target_texture(gfx_device_t *device, gfx_render_target_t *render_target, enum gfx_render_target_attachment attachment, const gfx_texture_t *texture)
{
}

static void d3d9_set_render_target_render_buffer(gfx_device_t *device, gfx_render_target_t *render_target, enum gfx_render_target_attachment attachment, const gfx_render_buffer_t *render_buffer)
{
}

static void d3d9_set_render_target_draw_buffers(gfx_device_t *device, gfx_render_target_t *render_target, uint32_t *draw_buffers, uint32_t draw_buffers_count)
{
}

static void d3d9_set_viewport(gfx_device_t *device, int32_t x, int32_t y, uint32_t width, uint32_t height)
{
...
static static void d3d9_set_scissor(gfx_device_t *device, int32_t x, int32_t y, uint32_t width, uint32_t height)
{
	RECT rect;
	rect.left = x;
	rect.right = x + width;
...
static void d3d9_set_line_width(gfx_device_t *device, float line_width)
{
	(void)device;
	(void)line_width;
}

static void d3d9_set_point_size(gfx_device_t *device, float pointSize)
{
...
{
	gfx_d3d9_device_t *device = (gfx_d3d9_device_t*)calloc(sizeof(*device), 1);
	if (!device)
		return NULL;
...
#endif
	if (FAILED(D3D9CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, creation_flags, &feature_level, 1, D3D11_SDK_VERSION, swap_chain_desc, swap_chain, (ID3D11Device**)&device->d3ddev, NULL, (ID3D11DeviceContext**)&device->d3dctx)))
	{
		if (window->error_callback)
			window->error_callback("failed to create d3d device");
		goto err;
	}
