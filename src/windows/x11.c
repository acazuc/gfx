...
{
	(void)display;
	return event->type == GenericEvent || event->xany.window == ((gfx_x11_window_t*)window)->window;
}
...
{
	(void)display;
	gfx_x11_window_t *win = (gfx_x11_window_t*)window;
...
void gfx_x11_set_clipboard(gfx_x11_window_t *window, const char *clipboard)
{
	...
	switch (event->type)
	{
		case ConfigureNotify:
			...
			if (window->winref->x != event->xconfigure.x || window->winref->y != event->xconfigure.y)
			{
				...
				{
					gfx_move_event_t evt;
					evt.used = false;
					evt.x = window->winref->x;
					evt.y = window->winref->y;
					window->winref->move_callback(&evt);
				}
			}
			break;
		case Expose:
			if (window->winref->expose_callback)
				window->winref->expose_callback();
			break;
		case KeyPress:
		{
				...
				if (len > 0 && (status == XLookupChars || status == XLookupBoth))
				{
					...
					if (utf8_decode((const char**)&iter, &evt.codepoint))
					{
						for (size_t i = 0; i < 5; ++i)
							evt.utf8[i] = text[i];
						window->winref->char_callback(&evt);
					}
					else
					{
			...
			if (gfx_is_key_down(window->winref, key))
			{
				if (window->winref->key_press_callback)
				{
					...
					window->winref->key_press_callback(&evt);
				}
			}
			else
			{
				...
				if (window->winref->key_down_callback)
				{
					...
					evt.key = key;
					evt.mods = get_mods(event->xkey.state);
					window->winref->key_down_callback(&evt);
				}
			}
			break;
		}
		case KeyRelease:
		{
			...
			if (window->winref->key_up_callback)
			{
				...
				evt.key = key;
				evt.mods = get_mods(event->xkey.state);
				window->winref->key_up_callback(&evt);
			}
			break;
		}
		case ButtonPress:
		...
		case MotionNotify:
			...
			{
				gfx_pointer_event_t evt;
				evt.used = false;
				evt.x = event->xmotion.x;
				evt.y = event->xmotion.y;
				window->winref->mouse_move_callback(&evt);
			}
			break;
		case FocusIn:
			if (window->winref->focus_in_callback)
				window->winref->focus_in_callback();
			break;
		case FocusOut:
			if (window->winref->focus_out_callback)
				window->winref->focus_out_callback();
			break;
		case EnterNotify:
			if (window->winref->cursor_enter_callback)
				window->winref->cursor_enter_callback();
			break;
		case LeaveNotify:
			if (window->winref->cursor_leave_callback)
				window->winref->cursor_leave_callback();
			break;
		case ClientMessage:
