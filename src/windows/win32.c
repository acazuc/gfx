	...
	switch (message)
	{
		...
		case WM_XBUTTONUP:
		{
			...
			if (down)
			{
				if (window->winref->mouse_down_callback)
				{
					...
					event.button = button;
					event.mods = get_mods();
					window->winref->mouse_down_callback(&event);
				}
			}
			else
			{
				if (window->winref->mouse_up_callback)
				{
					...
					event.button = button;
					event.mods = get_mods();
					window->winref->mouse_up_callback(&event);
				}
			}
			break;
		}
		...
		case WM_SYSKEYDOWN:
		{
			...
			if (window->winref->keys[key / 8] & (1 << (key % 8)))
			{
				if (window->winref->key_press_callback)
				{
					...
					event.key = key;
					event.mods = get_mods();
					window->winref->key_press_callback(&event);
				}
			}
			else
			{
				...
				if (window->winref->key_down_callback)
				{
					...
					event.key = key;
					event.mods = get_mods();
					window->winref->key_down_callback(&event);
				}
			}
			break;
		}
		...
		case WM_SYSKEYUP:
		{
			...
			if (window->winref->key_up_callback)
			{
				...
				event.key = key;
				event.mods = get_mods();
				window->winref->key_up_callback(&event);
			}
			break;
		}
		case WM_MOUSEMOVE:
		{
			...
			if (window->winref->mouse_move_callback)
			{
				...
				event.x = x;
				event.y = y;
				window->winref->mouse_move_callback(&event);
			}
			break;
		}
		case WM_INPUT:
		...
		case WM_MOUSEHWHEEL:
		{
			if (window->winref->scroll_callback)
			{
				...
				event.y = 0;
				window->winref->scroll_callback(&event);
			}
			break;
		}
		case WM_MOUSEWHEEL:
		...
		case WM_MOVE:
		{
			...
			if (x != window->winref->x || y != window->winref->y)
			{
				...
				if (window->winref->move_callback)
				{
					...
					event.x = x;
					event.y = y;
					window->winref->move_callback(&event);
				}
			}
			break;
		}
		case WM_SIZE:
		{
			...
			if (width != window->winref->width || height != window->winref->height)
			{
				...
				if (window->winref->resize_callback)
				{
					...
					event.width = width;
					event.height = height;
					window->winref->resize_callback(&event);
				}
			}
			break;
		}
